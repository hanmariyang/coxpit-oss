...
| Contract strip | `.contract` / `.req` (run modal, under the header) | `요청 산출물(계약): [icon type]… — 부수: …` from `GET /api/runs/:id/outputs`; `#i-check` present (green), `#i-alert-triangle` declared-but-missing (amber `.req.warn`), 부수 (incidental) is dashed `.req.aux`; hidden when a run declares nothing and produced no cards |
| Output cards | `#outCards` + `.ocard` (run modal right column) | one card per deliverable — a typed Lucide icon (`OUT_ICON`: answer→`#i-message` · code→`#i-code` · doc→`#i-file` · page/file→`#i-image`) + title + `요청됨`/`부수` badge + meta + `#i-chevron`; missing declared cards render dashed (`.miss`, "산출물 미충족") and are not clickable; `pickDefaultCard()` auto-opens the run's primary deliverable (doc/answer when no code, else the declared doc, else code, else first present) |
...
| Base branch dialog | `#brOverlay` (`.cfm` variant) + `#repoBranch` (`#i-branch`) in Context | merge · Sync base · PR all target it; validated against the repo (must exist); the repo `<option>` already shows the branch after the name |
