...
# v6.4 — 모바일 뷰어 재빌드(Grab & Read): <pre> 덤프 → 구조화된 줄 + 툴바(↵ ⌕ ↕) + 검색바 + 최신 FAB.
# 순서 = 서빙 HTML 순서(CSS → 마크업 → JS).
case "$CKPT" in *'.hln-url{'*'.hln-path{'*'.hist-fab{'*'id="histWrap"'*'id="histFind"'*'id="histJump"'*'id="histRefresh"'*'class="hist-findbar"'*'id="histLines"'*'class="hist-fab"'*) : ;; *) fail "v6.4: viewer toolbar (histWrap/histFind/histJump) + hist-findbar + #histLines + FAB markup missing";; esac
case "$CKPT" in *'histBody'*) fail "v6.4: the old <pre> scrollback dump (#histBody) should be gone — the terminal tab renders structured lines";; *) : ;; esac
case "$CKPT" in *'function linkify'*'class="hln-url"'*'class="hln-path"'*'function renderHistLines'*'class="hln-cp"'*'function renderTurn'*'class="hln-code"'*'class="cb-cp"'*) : ;; *) fail "v6.4: viewer line render path (linkify → renderHistLines with hln-cp → renderTurn code blocks) missing";; esac
case "$CKPT" in *'function histClick'*'openPathFromTerm(histRid'*'function runHistFind'*'mark'*'function stepHistFind'*'function histJump'*"'coxpit.histwrap'"*) : ;; *) fail "v6.4: viewer taps (path → openPathFromTerm), find, jump, or wrap persistence missing";; esac
...
' "$ROOT")
case "$CPT" in COPY_OK) : ;; *) fail "v6.4: copyText must be one shared helper used by terminal select-to-copy and the viewer: $CPT";; esac
