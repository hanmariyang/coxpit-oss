...
- [x] **Part B — stale-process guard** *(shipped v6.1.0 — r61)*: an on-demand listener inspector (`src/procscan.ts`, read-only `lsof`/`ps` over `runShellOn`) answers "what is LISTENing here, since when (etime), and is it under this pane's checkout (underPane)" — the DeskBox stale-build case — with an exact-pid `[종료]` kill (guarded server-side: never pid 1, the daemon's own pid, or a pid absent from a fresh re-scan). It **never declares a process stale** — facts, not verdicts. Plus passive port-spotting from the Part A tail (detection only, never invents a port).
- [x] **Part C — quick actions + safe context injection** *(shipped v6.1.0 — r62)*: context-aware quick replies on a `waiting` pane (승인·거절·계속 + a saved custom, fixed human-chosen strings over the existing `{t:'i'}` channel — never auto-answered from the prompt); one-tap 에이전트 시작 / 이어서 on an idle pane (reusing the reqbar launch/resume path); and file/selection injection fenced as reference data + previewed in the input, never auto-sent (ai-job-search injection discipline).
...
- [x] **Part D — input/output ergonomics** *(shipped v6.1.0)*: clickable terminal paths now resolve against the pane's live pwd (tmux `#{pane_current_path}`, `GET /api/runs/:id/pwd`), stitch wrapped rows, and the jail toast is a click target (§D-fix, r60); repo-row affordances overflow to a `⋯` menu so the project name is never crowded out by buttons (§D-rail, r60); and the mobile input bar now takes a **multi-line paste** — bracketed (`\e[200~..201~`), CRLF-normalized, no auto-submit (§D-io) — so a code block can be pasted on iPad/phone at all. *(Remaining: a mobile quick-action bar for the Part C affordances — later.)*
- [x] **Part E — run activity view** *(shipped on main, unreleased — r63)*: clicking a running agent in the rail opened an empty worktree shell, because headless agents run **detached** (not inside their tmux session) and the work lives in the parsed event stream. The run tab now carries an `activity | terminal` mode (agent → activity default, hand session/workbench → terminal default; the PTY is created lazily and retained, one-click toggle). The activity view shows, from data the client already has: header (role · repo ▸ task · Part A state chip · model · branch), a live "Now" line (`latestActivity`), the humanized timeline (tail-pinned auto-scroll), a `filesChanged` + diff-so-far peek, and an action bar (Open terminal · Steer · quick replies when waiting · Review). One shared `src/humanize.ts` now feeds both the board timeline and the activity pane. Read-first — the raw terminal is never taken away, only made explicit. *(Follow-up polish: the activity chrome is English (Changes/Latest/WS/Loading), and its buttons match the toolbar `.tc-btn` — a diff-peek button had been rendering as a native white button because `.abtn` was scoped too narrowly.)*
- [x] **Part F — rail header declutter + English labels** *(shipped on main, unreleased — r64)*: the Workspace header is `Workspace` + one `⋯` (Worktrees · Orphans behind it), the duplicate `local` is gone (the top-bar chip already shows it), and the section actions read English (Orphans · Tidy… · ＋ Session).
- [x] **Part G — uniform ⋯ headers + cockpit Add project** *(shipped on main, unreleased — r65)*: every header is `<name> + ⋯` — Scratch (＋ Session · Tidy…), Projects (**＋ Add project…** = the board's browse/register picker brought into the cockpit, so a project is born without leaving for the board), and the top bar (∗ Secrets · ◎ Notifications behind one `⋯`, with `● live` and `← Board` kept in the open). One shared menu mechanism behind every `⋯`; existing handlers/endpoints only.
- [x] **Part H — dry runs are a first-class property** *(shipped v6.3.0 — r70, found dogfooding)*: a dry (mock) run used to be indistinguishable from real work in a race. `agent_runs.real` is now stored and rides every run payload; a lowercase `dry` chip marks mock runs in the tree/activity/board (real runs get none), and comparing/merging a dry run asks first. Default real=1 so only known-dry runs are badged.
- [x] **Part I — the board excludes Scratch sessions** *(shipped v6.3.0 — r76)*: free terminal sessions (`repo.kind==='sessions'`) are not agent runs, so the board (the reading room for agent work) no longer lists them — filtered at every ingestion point (hydrate · WS deltas · repo list · counts · archive). `/api/fleet` and the cockpit are unchanged; Scratch still lives in the cockpit.
- [x] **Part J — auto-verify the merged winner** *(shipped v6.3.0 — r79, found dogfooding)*: merging a race winner now runs the repo's `verifyCmd` on the merged base and reports pass/fail right at the merge site (`verifyBase`, sharing `verifyRun`'s runner). A failing verify never un-merges; no `verifyCmd` shows a nudge to set one — so nobody hand-builds a smoke-test task every ticket.
- [x] **test tmux isolation** *(shipped v6.3.1)*: the e2e booted a daemon that ran `tmux new/kill-session` on the DEFAULT tmux server (`unset TMUX` only drops the client, not the socket dir), so its `coxpit-r<N>` ops collided with — and killed — real resident/user sessions. Fixed by pointing `TMUX_TMPDIR` at a per-run private dir (source unchanged — `runShellOn` inherits `process.env`, `term.spawnPty` spreads `{...process.env}`; production with the socket unset is byte-identical). A sentinel check proves the default server is never touched.
- [x] **Part K (page) — the desktop HUD, served page** *(shipped v6.3.1 — r80)*: `GET /hud` — a self-contained mono page (collapsed pill → triage list → side-by-side detail) reading the same `/api/fleet` + `/ws` + Part A/C/E/H, plus a minimal `POST /api/runs/:id/input` (tmux send-keys) to answer a waiting agent. Shared `src/activity.ts` (latestActivity) now feeds both cockpit and hud. **Rolled back 2026-09-22** — the page, the Electron window and the hotkey (§K1/K2) were all removed; the owner barely used the HUD and found it in the way. `POST /api/runs/:id/input` and `src/activity.ts` stay. See [design/v5.28-part-K-desktop-hud.md](../design/v5.28-part-K-desktop-hud.md).
...
- [x] **T6 — project-node hygiene** *(shipped v6.0.0 — r58)*: 등록 해제 / 묵은 작업 정리 on the project node, and an
